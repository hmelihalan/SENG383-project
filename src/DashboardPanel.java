...
public class DashboardPanel extends JPanel {
    ...
    private void showAddTaskDialog() {
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            // --- FIX: Find Max ID safely ---
            int maxId = 100;
            if (!TaskManager.tasks.isEmpty()) {
                // Task IDs are integers, so we can just find the max key
                maxId = Collections.max(TaskManager.tasks.keySet());
            }
            int newId = maxId + 1;
            // -------------------------------
    ...
    private void showAddWishDialog() {
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            // --- FIX: Find Max ID safely ---
            int maxId = 100; 
            // Iterate over existing String keys (e.g., "W102") to find the highest number
            for(String idStr : WishManager.wishes.keySet()) {
                try {
                    // Remove non-digits to extract "102" from "W102"
                    int currentId = Integer.parseInt(idStr.replaceAll("\\D", ""));
                    if(currentId > maxId) {
                        maxId = currentId;
                    }
                } catch (Exception e) {
